const hfa = 100.0; /*Konstanta za home field advantage*/
const k = 10;/*Konstanta za računanje novog eloa*/
...
let fixture; /*Ovdje ćemo spremati utakmicu koju dobijemo preko data parametra*/
let lineupContainer;
let statsContainer;

function OnLoadSimulator() {
  /*Provjerava postoji li parametar data te po određenoj vrsti (prediction ili gameinfo) prikazuje ili predickiju ili informaciju o utakmici*/
  if (data != null) {
    ...
    if (data.type == "prediction") {
      ...
      fetch("../src/data/teamlist.json")
        .then((res) => res.json())
        .then((teams) =>
          teams.data.forEach((el) => {
            if (el.id == data.data.hTeam || el.name == data.data.hTeam) {
              hTeam = el;
            } else if (el.id == data.data.aTeam || el.name == data.data.aTeam) {
              aTeam = el;
            }
          })
        )
        .then((res) => {
          if (hTeam == undefined || aTeam == undefined) {
            container.innerHTML =
              '<h1 class="text-white self-center">Trenutno nemamo ELOeve tih timova u našoj bazi</h1>';
          } else {
            Predict(
              [hTeam.name, hTeam.id, hTeam.image_path, hTeam.elo],
              [aTeam.name, aTeam.id, aTeam.image_path, aTeam.elo]
            );
          }
        });
    } else if (data.type == "gameinfo") {
      fetch(
        `https://api.sportmonks.com/v3/football/fixtures/${data.data}?api_token=${sportMonksToken}&include=participants;scores;statistics.type;lineups.type;`
      )
        .then((res) => res.json())
        .then((fixtureData) => {
...
/*Dodaje timove u izbornike za izabrat timove*/
function CreateSelectTeam() {
...
/*Po ELO formuli računa postatak pobjede za domaći tim*/
...
function PredictClick() {
  buttonContainer.classList.toggle("hidden");
  Predict(
    document.getElementById("selectHomeTeam").value.split("|"),
    document.getElementById("selectAwayTeam").value.split("|")
  );
}

/*U container stavlja predviđene rezulate za utakmicu i stavlja ih na grafički prikaz*/
function Predict(hTeam, aTeam) {
  let containerHome = document.getElementById("homeContainer");
  let containerAway = document.getElementById("awayContainer");
  ...
  AppendChildren(containerHome,[homeLabel, homePic])
  ...
  AppendChildren(containerAway,[awayLabel, awayPic])

  container.innerHTML = `
  <div class="self-center flex text-white flex-col">
    <h1>Domaći tim: ${hTeam[0]} ELO: ${homeTeamElo}</h1>
    <h2>Šanse: ${parseInt(homeTeamWinRate * 100)}%</h2>
  </div>
  <div class="self-center flex flex-col text-white">
    <h1>Gostujući tim: ${aTeam[0]} ELO: ${awayTeamElo}</h1>
    <h2>Šanse: ${parseInt(100 - homeTeamWinRate * 100)}%</h2>
  </div>`;
}

/*Briše iz containera statisku utakmice i stavlja postavu timova u utakmici*/
function ShowLineup() {
  /*Provjerava je li postava spremljena u varijablu lineupContainer, ako nije onda dobiva sve informacije potrebne i sprema ih u tu varijablu*/
...
/*Briše iz containera postavu timova i stavlja statistiku utakmice*/
function ShowStats() {
  /*Provjerava je li statistika spremljena u varijablu statsContainer, ako nije onda dobiva sve informacije potrebne i sprema ih u tu varijablu*/
  if (statsContainer == undefined) {
    let hTeam = fixture.participants[0].name;
    let aTeam = fixture.participants[1].name;

    let hTeamStats = fixture.statistics.filter((el) =>
      el.location == "home" ? true : false
    );
    let aTeamStats = fixture.statistics.filter((el) =>
      el.location == "away" ? true : false
    );

    let scores = fixture.scores.filter(
      (el) => (el.description = "CURRENT" ? true : false)
    );
    let hScore = scores.filter((el) => el.score.participant == "home")[0].score
      .goals;
    let aScore = scores.filter((el) => el.score.participant == "away")[0].score
      .goals;
    ...
    <div class="self-center mx-auto flex flex-col">
      ...
      <div class="grid grid-cols-3 gap-x-2">
        <h3>${
          hTeamStats.filter((el) => el.type.name == "Ball Possession %")[0].data.value
        }</h3>
        <h3>Posjed lopte</h3>
        <h3>${
          aTeamStats.filter((el) => el.type.name == "Ball Possession %")[0].data.value
        }</h3>
      </div>

      <div class="grid grid-cols-3 gap-x-2">
        <h3>${
          hTeamStats.filter((el) => el.type.name == "Yellowcards")[0].data.value
        }</h3>
        <h3>Žuti Kartoni</h3>
        <h3>${
          aTeamStats.filter((el) => el.type.name == "Yellowcards")[0].data.value
        }</h3>
      </div>

      <div class="grid grid-cols-3 gap-x-2">
        <h3>${
          hTeamStats.filter((el) => el.type.name == "Redcards")[0].data.value
        }</h3>
        <h3>Crveni Kartoni</h3>
        <h3>${
          aTeamStats.filter((el) => el.type.name == "Redcards")[0].data.value
        }</h3>
      </div>
      ...
      <div class="grid grid-cols-3 gap-x-2">
        <h3>${
          hTeamStats.filter((el) => el.type.name == "Offsides")[0].data.value
        }</h3>
        <h3>Offside</h3>
        <h3>${
          aTeamStats.filter((el) => el.type.name == "Offsides")[0].data.value
        }</h3>
      </div>
      ...
      <div class="grid grid-cols-3 gap-x-2">
        <h3>${
          hTeamStats.filter((el) => el.type.name == "Shots Total")[0].data.value
        }</h3>
        <h3>Ukupno šuteva</h3>
        <h3>${
          aTeamStats.filter((el) => el.type.name == "Shots Total")[0].data.value
        }</h3>
      </div>

      <div class="grid grid-cols-3 gap-x-2">
        <h3>${
          hTeamStats.filter((el) => el.type.name == "Shots On Target")[0].data.value
        }</h3>
        <h3>Šutevi u okvir</h3>
        <h3>${
          aTeamStats.filter((el) => el.type.name == "Shots On Target")[0].data.value
        }</h3>
      </div>

      <div class="grid grid-cols-3 gap-x-2">
        <h3>${
          hTeamStats.filter((el) => el.type.name == "Shots Off Target")[0].data.value
        }</h3>
        <h3>Šutevi izvan okvira</h3>
        <h3>${
          aTeamStats.filter((el) => el.type.name == "Shots Off Target")[0].data.value
        }</h3>
      </div>
    </div>`;
