...
function GetMatchups(year,month,day,page,league) {
  fetch(`https://api.sportmonks.com/v3/football/fixtures/date/${year}-${month}-${day}?api_token=${sportMonksToken}&page=${page}&per_page=50&include=scores;league;venue;statistics.type;participants;`).then(response => response.json()).then(data => {
    matchupList.push(...data.data)
    ParseMatchups(data.data, league);
    if(data.pagination.has_more)
    {GetMatchups(year,month,day,page+1)}
    else{sessionStorage.setItem('SHOWNMATCHUPS', JSON.stringify(matchupList))}
  });
}
...
function ParseMatchups(fixtures, league) {
  fixtures.forEach(fixture => {
    ...
    let buttonlink = document.createElement('a');  buttonlink.setAttribute('class', 'bg-[#1bb198] border-t-0 border-y-4 border-green-800 hover:bg-green-700 text-white font-bold my-3 p-2 text-center w-full rounded shadow-slate-900  shadow-md hover:shadow-green-500/50');
